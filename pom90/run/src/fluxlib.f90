...
subroutine surface_airseaflux
  ...
  do j=1,jm
     do i=1,im
        ...
        airt(i,j)=max((1.d0-ratio)*airt0(i,j)+ratio*airt1(i,j), -273.15d0)
        !airh(i,j)=(1.d0-ratio)*airh0(i,j)+ratio*airh1(i,j)
        airh(i,j)= max((1.d0-ratio)*airh0(i,j)+ratio*airh1(i,j), 0.d0)
        !lwrad(i,j)=(1.d0-ratio)*lwrad0(i,j)+ratio*lwrad1(i,j)
        lwrad(i,j)=max((1.d0-ratio)*lwrad0(i,j)+ratio*lwrad1(i,j), 0.d0)
        !swrad(i,j)=(1.d0-ratio)*swrad0(i,j)+ratio*swrad1(i,j)
        swrad(i,j)=max((1.d0-ratio)*swrad0(i,j)+ratio*swrad1(i,j), 0.d0)
        !slp(i,j)=(1.d0-ratio)*slp0(i,j)+ratio*slp1(i,j)
        slp(i,j)=max((1.d0-ratio)*slp0(i,j)+ratio*slp1(i,j), 0.d0)
        prep(i,j)=max((1.d0-ratio)*prep0(i,j)+ratio*prep1(i,j), 0.d0)
