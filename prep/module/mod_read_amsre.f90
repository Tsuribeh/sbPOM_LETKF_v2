...
contains
  ...
  subroutine read_amsre_filename(iyr,imon,iday,nfile,filename)
    ...
    !Count file
    nfile=0
    open(1,file="amsre"//yyyymmdd//".dat",status="old")
    do
       read(1,*,end=100)
       nfile=nfile+1
    end do
100 close(1)
    ...
    if(nfile /= 0)then
       ...
       close(1)
       status=system("rm -f amsre"//yyyymmdd//".dat")

    end if
    status=system("rm -f amsre"//yyyymmdd//".dat")

  end subroutine read_amsre_filename

  !-------------------

  subroutine read_amsre(filename,iyr,imon,iday,ihour,im,jm,long,lati,dat)
  ...
  subroutine deallocate_amsre(long,lati,dat)
  ...
  end subroutine deallocate_amsre

  !-----

  subroutine deallocate_amsre_filename(filename)
  ...
  end subroutine deallocate_amsre_filename

end module mod_read_amsre
