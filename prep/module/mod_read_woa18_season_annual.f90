module mod_read_woa18_season_annual

  integer,parameter :: im=360,jm=180,km=102
...
contains
  ...
  ! * 0: Annual mean
  ! * 18(1-3):Winter, 14(4-6):Spring, 15(7-9):Summer, 16(10-12):Autumn
  !
  ...
  subroutine read_woa18_season_annual(imon,long,lati,depth,t,s)
    ...
    real tmp1dx(im),tmp1dy(jm),tmp1dz(km)
    real ttmp(im,jm,km),stmp(im,jm,km)
    ...
    if(imon == 0)then
       jmon=0
    elseif(1 <= imon .and. imon <= 3)then
       jmon=13
    else if(4 <= imon .and. imon <= 6)then
       jmon=14
    else if(7 <= imon .and. imon <= 9)then
       jmon=15
    else if(10 <= imon .and. imon <= 12)then
       jmon=16
    end if
    ...
    if(status /= 0)then
       write(*,*) "***Error: Not found "//trim(tfilename)
       stop
    end if
    status=access(trim(sfilename)," ")
    if(status /= 0)then
       write(*,*) "***Error: Not found "//trim(sfilename)
       stop
    end if
    ...
    do itmp=1,im
       if(-180.e0 <= tmp1dx(itmp) .and. tmp1dx(itmp)< 0.)then
          i=i+1
  ...
  end subroutine read_woa18_season_annual

  !-----------------------------------------------------------

  subroutine read_woa18_season_annual_var(varname,imon,long,lati,depth,dat)
  ...
  end subroutine read_woa18_season_annual_var
  
  !-----------------------------------------------------------

  subroutine read_woa18_season_annual_err(imon,long,lati,depth,tse,sse)
    ...
    real tmp1dx(im),tmp1dy(jm),tmp1dz(km)
    real tsetmp(im,jm,km),ssetmp(im,jm,km)
    real tsdtmp(im,jm,km),ssdtmp(im,jm,km)

    real(kind = 8) tsd(im,jm,km),ssd(im,jm,km)
    ...
    integer,intent(in) :: imon
    real(kind = 8),intent(out) :: long(im),lati(jm),depth(km)
    real(kind = 8),intent(out) :: tse(im,jm,km),sse(im,jm,km)
  ...
  end subroutine read_woa18_season_annual_err

end module mod_read_woa18_season_annual
